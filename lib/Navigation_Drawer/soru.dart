...
class _FirePageState extends State<firePage> {
  ...
  Future<void> _besekle([DocumentSnapshot? documentSnapshot]) async {
    ...
    if (documentSnapshot != null) {
      _bes.text = documentSnapshot['Çok İyi'].toString();
      final double? vote = double.tryParse(_bes.text);
      await _vote.doc(documentSnapshot.id).update({"Çok İyi": (vote! + 1)});
    }
  }

  Future<void> _dortekle([DocumentSnapshot? documentSnapshot]) async {
    final nam = FirebaseAuth.instance.currentUser?.email.toString();
    ...
    final CollectionReference _vote = FirebaseFirestore.instance
        ...
        .collection(nam!);
    if (documentSnapshot != null) {
      _drt.text = documentSnapshot['İyi'].toString();
      final double? vote = double.tryParse(_drt.text);
      await _vote.doc(documentSnapshot.id).update({"İyi": (vote! + 1)});
    }
  }

  Future<void> _ucekle([DocumentSnapshot? documentSnapshot]) async {
    final nam = FirebaseAuth.instance.currentUser?.email.toString();
    ...
    if (documentSnapshot != null) {
      ...
      final double? vote = double.tryParse(_uc.text);
      await _vote.doc(documentSnapshot.id).update({"Orta": (vote! + 1)});
    }
  }

  Future<void> _ikiekle([DocumentSnapshot? documentSnapshot]) async {
    final nam = FirebaseAuth.instance.currentUser?.email.toString();
    ...
    if (documentSnapshot != null) {
      ...
      final double? vote = double.tryParse(_iki.text);
      await _vote.doc(documentSnapshot.id).update({"Kötü": (vote! + 1)});
    }
  }

  Future<void> _birekle([DocumentSnapshot? documentSnapshot]) async {
    final nam = FirebaseAuth.instance.currentUser?.email.toString();
    ...
    if (documentSnapshot != null) {
      ...
      final double? vote = double.tryParse(_bir.text);
      await _vote.doc(documentSnapshot.id).update({"Çok Kötü": (vote! + 1)});
    }
  }
                      ...
                      child: Row(
                        children: [
                          IconButton(
                              ...
                              onPressed: () => _besekle(documentSnapshot)),
                          IconButton(
                              icon: const Icon(Icons.add),
                              onPressed: () => _dortekle(documentSnapshot)),
                          // IconButton(
                          //     icon: const Icon(Icons.add),
                          //     onPressed: () =>
                          ...
                          IconButton(
                              icon: const Icon(Icons.add),
                              onPressed: () => _ucekle(documentSnapshot)),
                          IconButton(
                              icon: const Icon(Icons.add),
                              onPressed: () => _ikiekle(documentSnapshot)),
                          IconButton(
                              icon: const Icon(Icons.add),
                              onPressed: () => _birekle(documentSnapshot)),
                        ],
                      ),
                    ),
                  ),
